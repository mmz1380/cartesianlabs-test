...
def create_user_followers_table_if_not_exists(engine):
    create_table_query = """
...
def upsert_user(df, engine):
    ...
    for _, row in df.iterrows():
        ...
        insert_query = f"""
        INSERT INTO user_followers ({columns})
        VALUES ({values})
...
if __name__ == '__main__':
    ...
    create_user_followers_table_if_not_exists(engine)

    # Query to fetch all data from the city_location table
    query = "SELECT login, followers_url FROM users order by id;"

    # Read the data into a Pandas DataFrame (country + city)
    try:
        user_df = pd.read_sql(query, engine)  # Gets both country and city columns
        users = []

        # Loop over cities to fetch users from GitHub
        for index, row in user_df[:50].iterrows():
            login = row['login']
